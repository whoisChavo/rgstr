...
let rown = new Discord.MessageEmbed().setColor(Config.Embed.Color).setFooter(Config.Embed.Footer).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true}))
...
message.channel.send(rown.setDescription(`${kisi} İsimli Kullanıcı Başarılı Bir Şekilde ${message.author} Tarafından Sunucuya Kayıt Edildi`)).then(m => m.delete({timeout: Config.Embed.Timeout}));
message.react(Config.Emojis.Check);
